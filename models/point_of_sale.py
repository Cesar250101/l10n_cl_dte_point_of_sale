    ...
    def action_pos_order_invoice(self):
        ...
        for r in self:
            if r.document_class_id.sii_code not in [33, 34]:
                continue
            recs += r
        factura=super(POS, recs).action_pos_order_invoice()
        print(factura['res_id'])
        factura_id=self.env['account.invoice'].search([('id','=',factura['res_id'])])
        self.sii_document_number=factura_id.sequence_number_next
        return factura
